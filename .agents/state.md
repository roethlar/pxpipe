...
- One-port subscription routing Slices 1–4 are independently accepted. Slice 4
  ...
  `33bb29e`. That exact head is packaged, installed, healthy on the one local
  port, and passed both production offline parser checks; no output or live
  request ran.
...
- No local implementation, packaging, installation, or offline-validation
  blocker remains. The current reviewed service is healthy.
